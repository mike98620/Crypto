...
// 將 UTC 時間戳記轉換成台灣時間（UTC+8）的年月日時，不依賴執行環境的時區設定，
// 手動加 8 小時再讀取 UTC 欄位，確保無論程式跑在哪個時區的伺服器上結果都一致
function tsToTaipeiParts(ts){
...
function tsToTaipeiDateStr(ts){
  const p = tsToTaipeiParts(ts);
  return `${p.y}-${p.m}-${p.day}`;
}
function tsToTaipeiDateTimeStr(ts){
  const p = tsToTaipeiParts(ts);
  return `${p.y}-${p.m}-${p.day} ${p.h}:${p.min}`;
}
function nowTaipeiStr(){
  return tsToTaipeiDateTimeStr(Date.now()) + ' (台灣時間)';
}
...
async function fetchCoinGecko(id, days){
  ...
  // 一律轉換成台灣時間（UTC+8）顯示，避免與實際看到訊息的時間對不起來
  const isHourly = days <= 90;
  const dates = json.prices.map(p => isHourly ? tsToTaipeiDateTimeStr(p[0]) : tsToTaipeiDateStr(p[0]));
...
async function fetchBinance(symbol, interval, limit){
  ...
  const json = await res.json();
  const dates = json.map(k => tsToTaipeiDateTimeStr(k[0]));
...
// 遇到 429（請求太頻繁）時，等待後重試；每次重試等待時間遞增（8秒、16秒、24秒），其他錯誤直接拋出不重試
async function fetchWithRetry(fetchFn, retries=3, baseDelayMs=8000){
...
async function main(){
  console.log('開始檢查訊號...', nowTaipeiStr());
  ...
  for(const coin of COINS){
    ...
    // 每個幣種的請求之間間隔久一點，降低觸發 CoinGecko 429 限流的機率
    await sleep(3000);
  }
